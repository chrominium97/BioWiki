...
public class CreateUserAndBlog {
    ...
    private void authenticateUser() {
        ...
        editor.putString(BioWiki.WPCOM_USERNAME_PREFERENCE, mUsername);
        editor.putString(BioWiki.WPCOM_PASSWORD_PREFERENCE, BioWikiDB.encryptPassword(mPassword));
        editor.commit();
        mResponseHandler.setStep(Step.AUTHENTICATE_USER);
        // fire off a request to get an access token
        BioWiki.getRestClientUtils().get("me", mResponseHandler, mErrorListener);
    }
    ...
    private void createBlog() {
        ...
        BioWiki.getRestClientUtils().post(path, params, null, mResponseHandler, mErrorListener);
    }
        ...
        private void nextStep(JSONObject response) {
            try {
                if (mStep == Step.AUTHENTICATE_USER) {
                    mCallback.onStepFinished(Step.AUTHETICATE_USER);
